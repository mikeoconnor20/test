...
def get_mysql_connection(host, port, user, password, db):
    ...
    host = reader.get('--login-path=local_mike', 'host')
    user = reader.get('--login-path=local_mike', 'user')
    port = reader.get('--login-path=local_mike', 'port')
    password = reader.get('--login-path=local_mike', 'password')
    #end region
